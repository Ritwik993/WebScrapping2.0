...
async function run() {
  ...
  //   { timeout: 90000 }
  // );
  await page.goto("https://www.compass.com/homes-for-sale/alameda-county-ca/start=82/", {
    timeout: 90000,
  });
  let InfoArray = [];
  ...
  const amenities=Object.fromEntries(MapObject2);
    ...
    for (let i = 0; i < ans?.length; i++) {
      ...
      propertyHistory.push({ price, eventAndSource, date, appreciation: 0.0 });
    }
  ...
  fs.appendFile("page3.json", JSON.stringify(InfoArray), (err) => {
    if (err) throw err;
...
async function run100() {
  ...
  const propertyListingDetails=Object.fromEntries(MapObject);
  console.log(propertyListingDetails);
  await browser.close();
}

// run100();
async function run101() {
  const browser = await puppeteer.launch({ headless: false });
  const page = await browser.newPage();
  await page.goto(
    "https://www.compass.com/listing/25439-parklane-drive-hayward-ca-94544/1572807009604608657/"
